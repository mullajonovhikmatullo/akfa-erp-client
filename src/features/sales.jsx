  ...
  return (
    <>
      <div className="page-head">
        ...
        <antd.Segmented value={tab} onChange={setTab} options={[
...
const NewSaleFlow = () => {
  ...
  const submit = () => {
    if (items.length === 0) { antd.message.warning("Add at least one product"); return; }
    ...
    const sale = {
      ...
      branchId, customerId, priceMode, currency,
      items: items.map(({ id, ...rest }) => rest),
      paid: Number(paid) || 0,
    };
    dispatch({ type: "sales/create", sale });
    antd.notification.success({ message: "Sale created", description: `${items.length} items · ${fmt.fmtMoney(subtotal, currency)}` });
    setItems([]); setPaid(0);
  };
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 18 }}>
          {isSuper && (
            <div>
              ...
              <antd.Select value={branchId} onChange={setBranchId} style={{ width: "100%" }}
                options={branches.map(b => ({ value: b.id, label: b.name }))} />
            </div>
          )}
          <div>
            <div style={{ fontSize: 12, color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 6 }}>Customer</div>
            <antd.Select showSearch optionFilterProp="label" value={customerId} onChange={setCustomerId} style={{ width: "100%" }}
          ...
          <div>
            ...
            <antd.Radio.Group value={priceMode} onChange={e => setPriceMode(e.target.value)}>
            ...
            {items.map(it => {
              ...
              return (
                <div key={it.id} className="sales-grid row">
                  ...
                  <antd.Button type="text" icon={<icons.DeleteOutlined />} onClick={() => removeItem(it.id)} danger size="small" />
                </div>
              );
            })}
          </div>
      ...
      <div className="card" style={{ position: "sticky", top: 76 }}>
        ...
        <div className="col" style={{ gap: 10 }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <antd.Radio.Group size="small" value={currency} onChange={e => setCurrency(e.target.value)}>
        ...
        <div className="col" style={{ gap: 10 }}>
          ...
          <div>
            ...
            <antd.InputNumber value={paid} onChange={v => setPaid(v || 0)} style={{ width: "100%" }} step={100000} min={0}
