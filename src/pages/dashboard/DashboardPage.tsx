    ...
    <>
      <div className="page-head">
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <Button icon={<PlusIcon size={18} />} onClick={() => navigate(ROUTES.SALES)}>
            {t('dashboard.newSale')}
          </Button>
          <Button icon={<BoxArrowDownIcon size={18} />} onClick={() => navigate(ROUTES.PURCHASES)}>
            {t('dashboard.stockIn')}
          </Button>
          <Button icon={<ArrowClockwiseIcon size={18} className={isFetching ? 'ph-icon-spin' : undefined} />} onClick={refetchAll}>
            {t('common.refresh')}
          </Button>
          <Button type="primary" icon={<ChartLineUpIcon size={18} weight="bold" />} onClick={() => navigate(ROUTES.ANALYTICS)}>
