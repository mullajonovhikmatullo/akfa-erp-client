...
export const createExpenseSchema = (t: (key: string) => string) =>
  z
    .object({
      ...
      amount: z.number({ error: t('validation.amountInvalidType') }).positive(t('validation.amountPositive')),
      usdToUzsRate: z.number().positive(t('validation.amountPositive')).optional(),
      description: z.string().max(500).optional().or(z.literal('')),
      expenseDate: z.string().optional(),
    })
    .refine((value) => value.currency === 'USD' || Number.isInteger(value.amount), {
      message: t('validation.amountInteger'),
      path: ['amount'],
    })
    .refine((value) => value.currency === 'UZS' || value.usdToUzsRate !== undefined, {
