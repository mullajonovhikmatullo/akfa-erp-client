            ...
            <div className="container-page">
                ...
                <div className="benefits-layout">
                    ...
                    <div className="benefits-dashboard" aria-label={problem.dashboardLabel} data-reveal="scale">
                        <div className="benefits-stat benefits-stat--income">
                            <small>{problem.stats.income}</small><strong>346.7 <i>{t.dashboard.currency}</i></strong><em>+16%</em>
                            <svg viewBox="0 0 190 45" aria-hidden="true">
                                <path d="M2 37 C24 35 29 21 45 25 S67 35 84 24 S108 14 124 22 S151 30 188 7"/>
                            </svg>
                        </div>
                        <div className="benefits-stat benefits-stat--branches">
                            <small>{problem.stats.activeBranches}</small><strong>12</strong><em>+2</em></div>
                        <div className="benefits-stat benefits-stat--products">
                            <small>{problem.stats.productCount}</small><strong>1 248</strong>
                            <div className="benefits-bars" aria-hidden="true"><i/><i/><i/><i/><i/><i/></div>
                        </div>
                        <div className="benefits-stat benefits-stat--top">
                            ...
                            {problem.stats.products.map((product, index) => (
                                <p key={`product-${index}`}><i
                                    className="icons-search icon-size-13"/><span>{product}</span><b>{compactNumber.format(productValues[index] ?? 0)}</b>
                                </p>
                            ))}
                        </div>
