...
export function QuantityStepper({ value, max, unitLabel, onMinus, onPlus, onChange }: QuantityStepperProps) {
  ...
  return (
    <div className="u-items-center u-grid u-gap-4 u-grid-cols-quantity-actions-wide">
      <Button icon={<StoreIcon name="minus" size={16} />} onClick={onMinus} disabled={value <= MIN_QTY} className="u-h-30 u-p-0 u-w-30" />
      <InputNumber<number>
      ...
      />
      <Button icon={<StoreIcon name="plus" size={16} />} onClick={onPlus} disabled={value >= effectiveMax} className="u-h-30 u-p-0 u-w-30" />
      <span
