...
export function QuantityStepper({ value, max, unitLabel, onMinus, onPlus, onChange }: QuantityStepperProps) {
  ...
  return (
    <div className="u-items-center u-grid u-gap-4 u-grid-cols-quantity-wide">
      <Button icon={<StoreIcon name="minus" size={16} />} onClick={onMinus} disabled={value <= 1} className="u-h-30 u-p-0 u-w-30" />
      <InputNumber<number>
      ...
      />
      <Button icon={<StoreIcon name="plus" size={16} />} disabled={max > 0 && value >= max} onClick={onPlus} className="u-h-30 u-p-0 u-w-30" />
      <span
